...
set_config() {
	...
	if [ ! -f "$NGINXCONF".bak ]; then
		...
		[ "$result" == '0' ] && logsh "【$service】" "nginx未安装" && exit
		[ ! -f "$NGINXCONF" ] && logsh "【$service】" "未找到配置文件！" && exit
		[ -f "$NGINXCONF" ] && cp $NGINXCONF $NGINXCONF.bak
	...
	if [ ! -f "$PHPCONF".bak ]; then
		...
		[ -f "$PHPCONF" ] && cp $PHPCONF $PHPCONF.bak || exit
	...
	#修改端口号
	listenline=$(cat $NGINXCONF | grep -n listen | cut -d: -f1 | head -1)
	[ ! -z "$listenline" ] && sed -i ""$listenline"s#.*#        listen  $port;#" $NGINXCONF
